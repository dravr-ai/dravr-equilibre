...
- **StoredSleepSession** — DB-ready sleep model with is_nap, stages as JSONB
- **StoredRecoveryMetrics** — daily recovery/readiness scores
- **StoredHealthMetrics** — body composition and vitals
- **EventRecord** — base event model (polymorphic: workout, sleep)
- **WorkoutDetails / SleepDetails** — extend EventRecord
...
cargo clippy -p dravr-equilibre              # core library
cargo clippy -p dravr-equilibre-mcp          # MCP server
cargo clippy -p dravr-equilibre-server       # REST+MCP server
...
All errors MUST use `EquilibreError` with appropriate variants:
```rust
// GOOD
return Err(EquilibreError::MissingField { field: "user_id".into() });
return Err(EquilibreError::OutOfRange { field: "hr".into(), value: 300.0, min: 0.0, max: 250.0 });
