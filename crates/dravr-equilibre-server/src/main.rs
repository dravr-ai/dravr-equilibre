// ABOUTME: Unified CLI entry point for dravr-equilibre server
...
use dravr_equilibre_mcp::{build_tool_registry, McpServer};
use dravr_equilibre_server::router::build_router;
...
#[derive(Parser)]
#[command(
    name = "dravr-equilibre-server",
    version,
    about = "Health domain model server"
)]
struct Cli {
...
async fn serve_http(
    ...
    info!("dravr-equilibre server listening on {addr}");
