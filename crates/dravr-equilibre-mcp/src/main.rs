// ABOUTME: CLI entry point for the dravr-equilibre MCP server
...
#[derive(Parser)]
#[command(
    name = "dravr-equilibre-mcp",
    version,
    about = "Health domain MCP server"
)]
struct Cli {
    #[command(flatten)]
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    info!(
        "Starting dravr-equilibre MCP server (transport: {})",
